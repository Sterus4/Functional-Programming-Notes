...
Golden тестирование чем то похоже на Doctest. Суть метода также проста: давайте будем сохранять ***золотой*** вариант нашей работы программы, с которым в будущем мы сможем сравнивать последующуе изменения

![Golden test](assets/golden-tests.png)

Предположим, что мы написали приложение настолько, чтобы при определенном сценарии оно молгло моделировать поведение, которое нам необходимо. Сохраним в *golden test* 2 вещи:
* Входные данные, которые мы хотели бы протестировать
* Выходные данные, которые выдала программа, ***которую мы считаем правильной***

Обычно они хранятся в yml формате.

Далее будем развивать функционал нашей програмымы. И после каждого сохраняемого изменения, будем брать все голден тесты, брать из них входные данные, и сверять с выходными. Что может произойти?:
1. Выходные данные совпали, это значит, что наша программа до сих пор реализиует функционал эталона
2. Выходные данные не совпали:
...
Данный подход обладает безусловными преимеществами:
1. Если вывод программы очень большой, то будет плохо вставлять весь этот код в исходники некоторого unit-теста. Сохраним его в отдельный файл
...
Проблема довольно понятная: если несколько потоков выполняются одновременно, 
мы не можем определить, в какой порядке будут выполнены инструкции на процессоре.
...
```
Здесь функция **func()**, которая увеличиывает счетчик **i** на 1, 
...
из-за чего результат вывода на экран не определен. Нас это конечно не устраивает, и здесь мы можем увидеть
пример **Race condition detector**, который реализован на языке Go:

Запустим нашу программу с ключом **-race**:
```bash
$ go run -race racy.go
```
...
из-за диспетчеризации потоков. 
* Далее он перебирает все возможные трассы доступа к данным и ищет разницу между ними.
