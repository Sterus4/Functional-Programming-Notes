...
Тогда для проверки отдельных компонентов `f1` и `f2`, потребуется проверить
- 2<sup>8</sup> + 2<sup>8</sup> = 2<sup>9</sup> комбинаций.

Для полной проверки их взаимодействия `f1_2`, потребуется проверить
- 2<sup>8+8</sup> = 2<sup>16</sup> комбинаций.

То есть, в интеграционном тестировании ОДЗ представляет собой комбинацию из ОДЗ компонентов. А значит, граничных случаев больше, и проверить их сложнее.
Тем не менее, ограничиваться только модульным тестированием не получится, потому что оно не позволяет проверить, корректно ли модули взаимодействуют друг с другом.

Эти тесты решают разные задачи, и их нужно комбинировать.
